...
QWZDC2018RecHit::~QWZDC2018RecHit() { return; }
...
void QWZDC2018RecHit::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (auto it = psrcDid->begin(); it != psrcDid->end(); it++) {
    if (cname2str_.find(uint32_t(*it)) == cname2str_.end())
      continue;
  ...
  for (auto it = psrcDid->begin(); it != psrcDid->end(); it++) {
    if (energy.find(uint32_t(*it)) == energy.end())
      continue;
  ...
  if (bNpeak_) {
    int ts1 =
        std::distance(charge[std::string("hZDCP_HAD1")],
                      std::max_element(charge[std::string("hZDCP_HAD1")], charge[std::string("hZDCP_HAD1")] + NS));
    int ts2 =
        std::distance(charge[std::string("hZDCP_HAD2")],
                      std::max_element(charge[std::string("hZDCP_HAD2")], charge[std::string("hZDCP_HAD2")] + NS));
    ...
    ts1 = std::distance(charge[std::string("hZDCM_HAD1")],
                        std::max_element(charge[std::string("hZDCM_HAD1")], charge[std::string("hZDCM_HAD1")] + NS));
    ts2 = std::distance(charge[std::string("hZDCM_HAD2")],
                        std::max_element(charge[std::string("hZDCM_HAD2")], charge[std::string("hZDCM_HAD2")] + NS));
    if ((ts1 == 4 or ts1 == 5) and (ts2 == 4 or ts2 == 5)) {
