...
QWZDC2018Producer2::~QWZDC2018Producer2() { return; }
...
void QWZDC2018Producer2::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (auto it = digis->begin(); it != digis->end(); it++) {
    ...
    for (int i = 0; i < digi.samples(); i++) {
      ...
      if (bDebug_)
        std::cout << " !!! " << std::hex << did() << " CapId = " << digi[i].capid()
                  << " -> Ped = " << pedestal_[did()][digi[i].capid()] << "\n";
    }
    if (bDebug_)
      cout << __LINE__ << "\n";
    ...
    if (regular_fC[4] != 0)
      pratio5o4->push_back(regular_fC[5] / regular_fC[4]);
    else
      pratio5o4->push_back(-1.);
    if (regular_fC[5] != 0)
      pratio6o5->push_back(regular_fC[6] / regular_fC[5]);
    else
      pratio6o5->push_back(-1.);
